    ...
    override fun getProperties(id: Int): LiveData<PokeProperty>? {
        return propertyDao.getProperty(id)?.let {
            Transformations.map(it) { propertyObject ->
                propertyObject?.let { propertyItem ->
                    PokedexMapper
                        .propertyEntityAsDomainModel(propertyEntity = propertyItem)
                }
            }
        }
    }
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                propertyDao.insertAll(
                    PokedexMapper
                        .propertyResponseAsDatabaseModel(propertyRootResponse = pokeProperty)
                )
            } catch (e: Exception) {}
        }
    }
